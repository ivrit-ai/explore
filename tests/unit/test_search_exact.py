...
class TestExactSearchWithFilters:
    ...
    def test_combined_filters(self, in_memory_db):
        ...
        t2 = MockTranscript(
            source="podcast-a", episode="podcast-a/2024.06.01 Ep2",
            episode_title="Ep2", episode_date="2024-06-01",
            segments=[MockSegment(0.0, 3.0, "שלום יוני")]
        )
        t3 = MockTranscript(
            source="podcast-b", episode="podcast-b/2024.06.01 Ep1",
            episode_title="Ep1", episode_date="2024-06-01",
        ...
        hits = index.search_hits(
            "שלום", search_mode='exact',
            sources=["podcast-a"], date_from="2024-03-01"
        )
        assert len(hits) == 1
