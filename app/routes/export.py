...
def export_results_csv():
    ...
    logger.info(
        f"Performing search for CSV export: {query} (mode: {search_mode}, filters: date_from={date_from}, date_to={date_to}, sources={sources}, ignore_punct={ignore_punct}, position_filters={position_filters})"
    )
    ...
    for i, hit in enumerate(hits):
        ...
        context_indices = []
        for offset in range(
            -DEFAULT_CONTEXT_SEGMENTS_LENGTH, DEFAULT_CONTEXT_SEGMENTS_LENGTH + 1
        ):  # -5 to +5 inclusive
    ...
    segment_map = {}
    for (ep_idx, seg_idx), seg_data in zip(segments_to_fetch, fetched_segments, strict=False):
        key = f"{ep_idx}|{seg_idx}"
        segment_map[key] = seg_data

    # Now build results with context
    for i, _hit in enumerate(hits):
    ...
    if position_filters:
        labels = {"start": "Beginning", "end": "End", "cross": "Cross-segment"}
        writer.writerow(["# Position Filter:", ", ".join(labels[p] for p in sorted(position_filters))])
    writer.writerow(["# Total Results:", len(all_results)])
    writer.writerow(["# Exported:", datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
    writer.writerow([])  # Empty row for separation

    # Write column headers
    writer.writerow(["Episode Index", "Date", "Source", "Episode", "Text", "Context", "Start Time", "End Time"])
    ...
    if analytics:
        analytics.capture_export(export_type="csv", query=query, execution_time_ms=execution_time)
...
def export_segment(source, filename):
    ...
    try:
        ...
        if not audio_path:
            logger.error(f"Audio file not found for: {source}/{filename}.opus")
