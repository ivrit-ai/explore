...
from .db import SQLITE_MAX_PARAMS, DatabaseService
...
def _classify_hit_position(
    hit_start: int, hit_end: int, full_text: str, seg_boundaries: list[tuple[int, int]], seg_offsets: list[int]
) -> set[str]:
...
class TranscriptIndex:
    ...
    def get_episode_by_uuid(self, doc_uuid: str) -> str:
        ...
        cursor = self._db.execute("SELECT episode FROM documents WHERE uuid = ?", [doc_uuid])
        result = cursor.fetchone()
        if not result:
    ...
    def get_segments_by_ids(self, lookups: list[tuple[int, int]]) -> list[dict]:
        ...
        for i in range(0, len(lookups), BATCH_SIZE):
            ...
            self._db.execute(f"INSERT OR IGNORE INTO temp_segment_lookups VALUES {placeholders}", params)
    ...
    def get_source_by_episode_idx(self, episode_idx: int) -> str:
        ...
        doc_id = episode_idx
        cursor = self._db.execute("SELECT source FROM documents WHERE doc_id = ?", [doc_id])
        result = cursor.fetchone()
        if not result:
            raise IndexError(f"Document {doc_id} not found")
        return result[0]
    ...
    def _get_segment_boundaries(self, doc_id: int) -> list[tuple[int, int]]:
        ...
        cursor = self._db.execute(
            "SELECT char_offset, LENGTH(segment_text) FROM segments WHERE doc_id = ? ORDER BY char_offset", [doc_id]
        )
    ...
    def _search_sqlite_simple(
        ...
        log.info(
            f"Searching for query: {query}, mode: {search_mode}, date_from: {date_from}, date_to: {date_to}, sources: {sources}"
        )
    ...
    def _search_fts5(
        ...
        log = logging.getLogger("index")
        log.info(
            f"FTS5 search: query={query}, mode={search_mode}, ignore_punct={ignore_punct}, position_filters={position_filters}"
        )
    ...
    def _search_fts5_regex(self, query: str, date_from, date_to, sources, position_filters: set[str] | None = None):
...
class IndexManager:
    ...
    def __init__(
        self, file_records: list[FileRecord] | None = None, index_path: Path | None = None, **db_kwargs
    ) -> None:
    ...
    @staticmethod
    def split_episode(episode_str: str) -> tuple[str | None, str]:
        """
    ...
    def _build(self) -> TranscriptIndex:
        ...
        def load_worker(rec_idx, rec):
            ...
            document_row = (rec_idx, doc_uuid, source, rec.id, episode_date, episode_title)
        ...
        def writer_thread():
            ...
            while True:
                ...
                if len(batch_docs) >= DOC_BATCH_SIZE:
                    ...
                    # Insert mappings
                    db.batch_execute("INSERT INTO fts_doc_mapping(fts_rowid, doc_id) VALUES (?, ?)", batch_mapping)
        ...
        db.execute(
            """CREATE INDEX idx_documents_date
                    ON documents(episode_date)"""
        )
        ...
        db.execute(
            """CREATE INDEX idx_fts_doc_mapping_doc_id
                    ON fts_doc_mapping(doc_id)"""
        )
...
def segment_for_hit(index: TranscriptIndex, episode_idx: int, char_offset: int) -> Segment:
